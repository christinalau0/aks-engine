...
func (uc *upgradeCmd) loadCluster() error {
	...
	// Only containerd runtime is allowed for Kubernetes 1.24+
	if strings.EqualFold(uc.containerService.Properties.OrchestratorProfile.KubernetesConfig.ContainerRuntime, "docker") && common.IsKubernetesVersionGe(uc.upgradeVersion, "1.24.0") {
